...
import { VOL2_BACKTEST_ID, VOL1_BACKTEST_ID, MOVED_TP_BACKTEST_ID, NOT_MOVED_TP_BACKTEST_ID } from '../src/backtests.js';
...
test('renderBacktestTabs shows tabs when Vol 2 is active and hides tabs when Vol 1 or custom is active', () => {
  const tab1 = element();
  ...
  // When Vol 2 is active with activeTabId moved-tp
  renderBacktestTabs({
...
test('renderBacktestSelect populates top-level backtest options and disables delete when 1 backtest', () => {
  ...
  const backtests = [
    { id: VOL2_BACKTEST_ID, name: 'Gold Backtest Vol. 2' },
    { id: VOL1_BACKTEST_ID, name: 'Gold Backtest Vol. 1' }
  ];
