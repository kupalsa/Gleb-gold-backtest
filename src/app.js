...
const syncTakeProfitMoveForm = () => syncTakeProfitMoveFields({ moved: $('#movedTakeProfit'), result: $('#takeProfitMoveResult'), container: $('#take-profit-move-result') });
...
function renderDay() {
  ...
  const template = $('#trade-row'); trades.forEach((trade) => { const row = template.content.firstElementChild.cloneNode(true); const duration = durationInMinutes(trade.entryTime, trade.exitTime, trade.date, trade.exitDate); row.querySelector('.trade-meta').innerHTML = `<strong>${trade.entryTime}–${trade.exitTime}</strong>${trade.exitDate ? `<span>Exit date: ${escapeText(trade.exitDate)}</span>` : ''}<span class="pill ${trade.direction}">${trade.direction}</span><span class="pill ${trade.outcome}">${trade.outcome}</span>${trade.notes ? `<p>${escapeText(trade.notes)}</p>` : ''}`; row.querySelector('.trade-values').textContent = `${formatDuration(duration)} · SL ${trade.stopLossPoints} gold points/pips · R:R ${trade.riskReward}`; row.querySelector('.edit').addEventListener('click', () => startEdit(trade)); row.querySelector('.delete').addEventListener('click', async () => { if (confirm(`Delete the ${trade.entryTime} ${trade.direction} trade?`)) try { await store.remove(trade.id); await refresh(); } catch (error) { setStatus('error', error.message); } }); list.append(row); });
}
function renderStats() {
  ...
  if (!state.trades.length) { const row = document.createElement('tr'); const cell = document.createElement('td'); cell.colSpan = 9; cell.className = 'muted'; cell.textContent = 'No trades recorded.'; row.append(cell); body.append(row); return; }
  [...state.trades].sort((a, b) => `${a.date} ${a.entryTime}`.localeCompare(`${b.date} ${b.entryTime}`)).forEach((trade) => {
    const row = document.createElement('tr'); const duration = /^([01]\d|2[0-3]):[0-5]\d$/.test(String(trade.exitTime || '')) ? formatDuration(durationInMinutes(trade.entryTime, trade.exitTime, trade.date, trade.exitDate)) : '—';
    [trade.date, trade.entryTime, trade.exitDate ? trade.exitDate : '', trade.exitTime || '—', duration, trade.direction, trade.outcome || '—', trade.stopLossPoints, trade.riskReward].forEach((value) => { const cell = document.createElement('td'); cell.textContent = value; row.append(cell); }); body.append(row);
  });
}
